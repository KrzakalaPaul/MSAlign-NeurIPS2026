...
def _result(args, config, validation, test):
    ...
    return {
        "status": "completed", "model": "FLARE", "dataset": args.labelled_dataset_name,
...
def train_and_eval_FLARE(args, config):
    ...
    trainer = Trainer(
        ...
        max_epochs=int(config["n_epochs_train"]),
        callbacks=callbacks,
    ...
    trainer.fit(
        ...
        ckpt_path=getattr(args, "resume_checkpoint", None),
    )
