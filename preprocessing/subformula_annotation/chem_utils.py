...
ion_to_mass = {
    ...
    "[M-H]-":        -ELEMENT_TO_MASS["H"] + ELECTRON_MASS,            # ← fixed sign on H
    "[M+NH4]+":       ELEMENT_TO_MASS["N"] + ELEMENT_TO_MASS["H"] * 4 - ELECTRON_MASS,
    "[M+HCOOH-H]-":   ELEMENT_TO_MASS["H"] + ELEMENT_TO_MASS["C"] + ELEMENT_TO_MASS["O"] * 2 + ELECTRON_MASS,  
    "[M+CH3COOH-H]-": ELEMENT_TO_MASS["H"] * 3 + ELEMENT_TO_MASS["C"] * 2 + ELEMENT_TO_MASS["O"] * 2 + ELECTRON_MASS, 
    "[M+Cl]-":        ELEMENT_TO_MASS["Cl"] + ELECTRON_MASS,         
}

ion_to_add_vec = {
    # --- existing (all correct) ---
    ...
    # --- missing negative mode adducts ---
    "[M-H]-":        -element_to_position["H"],
    "[M+HCOOH-H]-":  element_to_position["H"] + element_to_position["C"] + element_to_position["O"] * 2,   # HCOOH - H = H + C + 2O
    "[M+CH3COOH-H]-": element_to_position["H"] * 3 + element_to_position["C"] * 2 + element_to_position["O"] * 2,  # CH3COOH - H = 3H + 2C + 2O
    "[M+Cl]-":       element_to_position["Cl"],

    # --- missing positive mode adduct ---
    "[M+NH4]+":      element_to_position["N"] + element_to_position["H"] * 4,  # same atoms as [M+H3N+H]+
}
